...
def aggregate(rumors):
    ...
    reporters = defaultdict(lambda: {
        ...
        # team/player counts broken out by month. This is what makes period
        # filtering real: "Wolves mentions in March" is a stored number, not
        # a share of the reporter's March activity.
        "by_team_month": defaultdict(lambda: defaultdict(int)),
        "by_player_month": defaultdict(lambda: defaultdict(int)),
    })
    ...
    for rumor in rumors:
        ...
        for team in extract_teams(tags):
            s["by_team"][team] += 1
            if month:
                s["by_team_month"][team][month] += 1
        for player in extract_players(tags):
            s["by_player"][player] += 1
            if month:
                s["by_player_month"][player][month] += 1
...
def _plain(d):
    out = {}
    for k, v in d.items():
        if k in ("by_team_month", "by_player_month"):
...
def merge(part_aggs):
    ...
    for agg in part_aggs:
        ...
        for key, s in agg["reporters"].items():
            ...
            for field in ("by_team_month", "by_player_month"):
                for subject, months in (s.get(field) or {}).items():
                    _add_counts(tgt[field].setdefault(subject, {}), months)
    ...
    for key, s in reporters.items():
        ...
        # keep the heavy per-month detail out of the main payload
        if s.get("by_team_month"):
            team_months[key] = s["by_team_month"]
        if s.get("by_player_month"):
            player_months[key] = {p: m for p, m in s["by_player_month"].items()}
    ...
    return {
        ...
        "_team_months": team_months,
        "_player_months": player_months,
    }
...
def write_side_file(path, varname, payload):
    """Write one of the lazy-loaded detail files (teams-data.js / players-data.js).
    Only the page that needs it loads it, which keeps the leaderboard light."""
    body = json.dumps(payload, separators=(",", ":"), ensure_ascii=False)
    with open(path, "w", encoding="utf-8") as f:
        f.write("// generated by reporter_rankings_build.py - do not edit\n")
        f.write("const %s = %s;\n" % (varname, body))
        f.write("if (typeof window !== 'undefined') { window.%s = %s; }\n" % (varname, varname))
    return os.path.getsize(path)


def qa_report(data, team_months, player_months):
    """Data-integrity pass. Never fails the build: it writes a report so bad
    data is visible instead of silently shipping."""
    import process_archive as pa

    players = set()
    for r in data["reporters"]:
        players.update(p.lower() for p in (r.get("by_player") or {}))
    teams = set(t.lower() for t in getattr(pa, "NBA_TEAMS", []))

    checks = {
        "unknown_outlet": [], "name_matches_team": [], "malformed_name": [],
        "duplicate_canonical": [], "suspicious_handle": [],
        "period_exceeds_total": [], "subject_exceeds_reporter_total": [],
    }

    seen_lower = {}
    for r in data["reporters"]:
        n, total = r["name"], r["total"]
        if r.get("outlet") in (None, "", "Unknown"):
            checks["unknown_outlet"].append([n, total])
        if n.lower() in teams:
            checks["name_matches_team"].append([n, total])
        if len(n) < 3 or n.count(" ") > 4 or re.search(r"[0-9]{3,}|http|\.com", n, re.I):
            checks["malformed_name"].append([n, total])
        seen_lower.setdefault(n.lower().replace(".", "").replace("  ", " "), []).append(n)

        dated = sum((r.get("by_date") or {}).values())
        if dated > total:
            checks["period_exceeds_total"].append([n, dated, total])

        for bucket, src in (("by_team_month", team_months), ("by_player_month", player_months)):
            months = (src.get(r["id"]) or {})
            for subject, m in months.items():
                if sum(m.values()) > total:
                    checks["subject_exceeds_reporter_total"].append([n, subject, sum(m.values()), total])

    for low, variants in seen_lower.items():
        if len(set(variants)) > 1:
            checks["duplicate_canonical"].append(sorted(set(variants)))

    for key, entry in getattr(pa, "REPORTERS_DB", {}).items():
        for h in entry.get("handles", []):
            if re.search(r"aborz", h, re.I) or len(h) < 3:
                checks["suspicious_handle"].append([entry["name"], h])
...
def main():
    ...
    try:
        ...
        team_months = data.pop("_team_months", {})
        player_months = data.pop("_player_months", {})

        write_js(data, OUTPUT_JS)
        t_size = write_side_file("teams-data.js", "TEAM_MONTHS", team_months)
